Shader "Hidden/Owlcat Render Pipeline/LutBuilderHdr"
{
    HLSLINCLUDE
        ...
        #include "Common.hlsl"
        #include "Packages/com.wrath.core/ShaderLibrary/ACES.hlsl"
		...
		float4 _Slope;				// xyz: color, w: unused
		float4 _Power;				// xyz: color, w: unused
		float4 _Offset;				// xyz: color, w: unused
        ...
        float3 ColorGrade(float3 colorLutSpace)
        {
			...
			colorLog = OffsetPowerSlope(colorLog, _Offset.rgb, _Power.rgb, _Slope.rgb);
        ...
        float3 Tonemap(float3 colorLinear)
        {
            #if _TONEMAP_NEUTRAL
            {
                colorLinear = NeutralTonemap(colorLinear, _NeutralTonemapperParams1, _NeutralTonemapperParams2);
            }
            #elif _TONEMAP_ACES
            {
